...
export function FleetMap({ robots }: Props) {
  ...
  return (
    <div className="overflow-auto rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <h2 className="mb-3 text-sm font-semibold text-slate-500 uppercase tracking-wide">フロアマップ</h2>
      <svg
        viewBox={`0 0 ${W} ${H}`}
          ...
          return (
            ...
            >
              ...
              {(robot.status === 'LOW_BATTERY' || robot.battery_level < 20) && (
                <text x={cx + 10} y={cy - 10} fontSize={10}>⚡</text>
              )}
            </g>
          )
        })}
...
const STATUS_LABELS: Record<RobotStatus, string> = {
  IDLE: '待機中', CLEANING: '掃除中', CHARGING: '充電中',
  RETURNING_TO_DOCK: 'ドックへ', LOW_BATTERY: '低バッテリー',
  UPDATING: 'OTA更新中', ERROR: 'エラー',
}
