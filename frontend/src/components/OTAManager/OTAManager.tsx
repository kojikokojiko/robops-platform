...
const STATUS_LABEL: Record<string, string> = {
  QUEUED: '待機中', IN_PROGRESS: '更新中', SUCCEEDED: '完了', FAILED: '失敗', CANCELED: 'キャンセル',
}
...
export function OTAManager({ robots }: Props) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['ota-jobs'] }); setOpen(false) },
  })
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">
        {robots.map((r) => (
          <div key={r.robot_id} className="rounded-xl border border-slate-200 bg-white p-3 text-center">
      ...
      <div className="rounded-xl border border-slate-200 bg-white">
        ...
        {jobs.length === 0 ? (
          ...
          <div className="divide-y divide-slate-50">
            {jobs.map((job) => (
              <div key={`${job.job_id}-${job.robot_id}`} className="flex items-center justify-between px-4 py-3">
                <div>
                  <p className="text-sm font-medium text-slate-800">
                    {job.robot_id} &nbsp;
                    <span className="font-normal text-slate-500">→ v{job.firmware_version}</span>
                  </p>
                  <p className="text-xs text-slate-400">速度: {job.new_speed} m/s &nbsp;·&nbsp; {job.job_id}</p>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_COLOR[job.status] ?? 'bg-slate-100 text-slate-600'}`}>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <p className="mb-4 text-sm text-slate-500">速度変更をファームウェア更新としてロボットに配信します</p>
            <div className="space-y-3">
              ...
              <label className="block">
                <span className="text-sm text-slate-600">新しい最大速度 (m/s): <strong>{form.new_speed}</strong></span>
                <input
                ...
                />
                <div className="flex justify-between text-xs text-slate-400"><span>0.1</span><span>2.0</span></div>
              </label>
              <label className="block">
